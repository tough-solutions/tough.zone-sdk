    ...
    describe("static functions", () => {
        ...
        describe("createTickets", () => {
            it("should call API to create tickets", async () => {
                ...
                await expect(ToughZone.Tickets.createTickets(42)).resolves.toEqual([]);
            ...
            it("should call API to create tickets if amount is expressed as string", async () => {
                ...
                await expect(ToughZone.Tickets.createTickets("42" as unknown as number)).resolves.toEqual([]);
                expect(getGraphQlClientSpy).toBeCalled();
                expect(graphQlClientMock.mutate).toBeCalledWith({
                    mutation: createTickets,
                    variables: {
            ...
            it("should reject if no valid result is returned from the API", async () => {
                ...
                await expect(ToughZone.Tickets.createTickets(42)).rejects.toEqual(new Error(errors.noTicketsReturned));
                expect(getGraphQlClientSpy).toBeCalled();
                expect(graphQlClientMock.mutate).toBeCalledWith({
                    mutation: createTickets,
                    variables: {
            ...
            it("should reject with GraphQL errors", async () => {
                ...
                await expect(ToughZone.Tickets.createTickets(42)).rejects.toEqual(
            ...
            it("should throw if amount is not a positive integer", () => {
                expect(() => ToughZone.Tickets.createTickets(undefined as unknown as number)).toThrowError(
                    new Error(`"amount" is required`),
                );
                expect(() => ToughZone.Tickets.createTickets(-1)).toThrowError(
                    new Error(`"amount" must be a positive number`),
                );
                expect(() => ToughZone.Tickets.createTickets(1.5)).toThrowError(
        ...
        describe("deleteTickets", () => {
            it("should resolve with true if tickets were deleted", async () => {
                ...
                await expect(ToughZone.Tickets.deleteTickets([uuid])).resolves.toEqual(true);
                expect(getGraphQlClientSpy).toBeCalled();
                expect(graphQlClientMock.mutate).toBeCalledWith({
            ...
            it("should resolve with false if tickets were not deleted", async () => {
                ...
                await expect(ToughZone.Tickets.deleteTickets([uuid])).resolves.toEqual(false);
                expect(getGraphQlClientSpy).toBeCalled();
                expect(graphQlClientMock.mutate).toBeCalledWith({
                    mutation: deleteTickets,
                    variables: {
            ...
            it("should reject with GraphQL errors", async () => {
                ...
                await expect(ToughZone.Tickets.deleteTickets([uuid])).rejects.toEqual(
                    new Error(errors.mutationThrewMultipleErrors),
                );
                expect(getGraphQlClientSpy).toBeCalled();
                expect(graphQlClientMock.mutate).toBeCalledWith({
            ...
            it("should resolve with false if ticketUuids are empty", () =>
                expect(ToughZone.Tickets.deleteTickets([])).resolves.toBe(false));

            it("should throw if ticketUuids is not a valid array of UUIDs", () => {
                expect(() => ToughZone.Tickets.deleteTickets(undefined as unknown as string[])).toThrowError(
                    new Error(`"ticketUuids" is required`),
                );
                expect(() => ToughZone.Tickets.deleteTickets([1, 2] as unknown as string[])).toThrowError(
                    new Error(`"[0]" must be a string`),
                );
                expect(() => ToughZone.Tickets.deleteTickets([null] as unknown as string[])).toThrowError(
                    new Error(`"[0]" must be a string`),
                );
                expect(() => ToughZone.Tickets.deleteTickets([undefined] as unknown as string[])).toThrowError(
                    new Error(`"ticketUuids" must not be a sparse array item`),
                );
                expect(() => ToughZone.Tickets.deleteTickets(["x", "y"])).toThrowError(
